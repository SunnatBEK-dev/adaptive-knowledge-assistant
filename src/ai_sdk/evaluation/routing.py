...
class RouteEvalCase:
    id: str
    input_text: str
    expected_route: SuperAIRoute

    def __post_init__(self) -> None:
        if (
            not isinstance(self.id, str)
            or _NAME_PATTERN.fullmatch(self.id) is None
        ):
            raise EvaluationValidationError(
                "Route evaluation case ID is invalid."
            )
        if (
            ...
            raise EvaluationValidationError(
                "Route evaluation input cannot be empty."
            )
        if not isinstance(self.expected_route, SuperAIRoute):
            raise EvaluationValidationError(
                "Expected Super AI route is invalid."
            )
...
class RouteEvalResult:
    case_id: str
    expected_route: SuperAIRoute
    actual_route: SuperAIRoute | None
    ...
    def __post_init__(self) -> None:
        ...
        if not isinstance(self.expected_route, SuperAIRoute):
            raise EvaluationValidationError(
                "Route result expected route is invalid."
            )
        if self.actual_route is not None and not isinstance(
            ...
            raise EvaluationValidationError(
                "Route result actual route is invalid."
            )
        ...
        if any(
            ...
            raise EvaluationValidationError(
                "Route result signals are invalid."
            )
        if (
            ...
            raise EvaluationValidationError(
                "Estimated model request count is invalid."
            )
        if (
        ...
        if self.error_type is not None and (
            ...
            raise EvaluationValidationError(
                "Route evaluation error type is invalid."
            )
    ...
    def passed(self) -> bool:
        return (
            ...
            and self.actual_route is self.expected_route
        )
    ...
    def to_dict(self) -> dict[str, object]:
        return {
            ...
            "actual_route": (
                None
                if self.actual_route is None
                else self.actual_route.value
            ),
            "signals": [signal.value for signal in self.signals],
            "passed": self.passed,
            "expected_model_requests": (
                self.expected_model_requests
            ),
            "estimated_model_requests": (
                self.estimated_model_requests
            ),
...
class RouteEvaluationReport:
    ...
    def __init__(
        ...
        if not normalized or any(
            not isinstance(result, RouteEvalResult)
            for result in normalized
        ):
            raise EvaluationValidationError(
                "Route evaluation report requires results."
            )
        case_ids = [result.case_id for result in normalized]
        if len(case_ids) != len(set(case_ids)):
            raise EvaluationValidationError(
                "Route evaluation case IDs must be unique."
            )
    ...
    def passed(self) -> bool:
        return (
            self.error_cases == 0
            and self.accuracy >= self.minimum_accuracy
        )
    ...
    def expected_model_requests(self) -> int:
        return sum(
            result.expected_model_requests
            for result in self.results
        )
    ...
    def estimated_model_requests(self) -> int:
        return sum(
            result.estimated_model_requests
            for result in self.results
        )
    ...
    def full_route_baseline_requests(self) -> int:
        return self.total_cases * RoutingDecision(
            SuperAIRoute.FULL
        ).estimated_model_requests
    ...
    def estimated_request_savings(self) -> int:
        return (
            self.full_route_baseline_requests
            - self.estimated_model_requests
        )
    ...
    def per_route_accuracy(self) -> Mapping[str, float]:
        ...
        for route in SuperAIRoute:
            ...
            if matching:
                values[route.value] = sum(
                    result.passed for result in matching
                ) / len(matching)
        return values
    ...
    def to_dict(self) -> dict[str, object]:
        return {
            ...
            "full_route_baseline_requests": (
                self.full_route_baseline_requests
            ),
            "estimated_request_savings": (
                self.estimated_request_savings
            ),
            "mean_routing_latency_ms": (
                self.mean_routing_latency_ms
            ),
...
class RouteEvaluationRunner:
    ...
    def __init__(
        ...
        *,
        ...
        if not callable(clock):
            raise EvaluationValidationError(
                "Route evaluation clock must be callable."
            )
        self.router = router
    ...
    def evaluate(
        ...
        if not case_list or any(
            not isinstance(case, RouteEvalCase)
            for case in case_list
        ):
            raise EvaluationValidationError(
                "Route evaluation dataset is invalid."
            )
    ...
    def _elapsed_ms(self, started_at: float) -> float:
        ...
        if finished_at < started_at:
            raise EvaluationValidationError(
                "Route evaluation clock moved backwards."
            )
    ...
    def _clock_value(self) -> float:
        ...
        if (
            ...
            raise EvaluationValidationError(
                "Route evaluation clock value is invalid."
            )
        return float(value)
...
def _validate_accuracy(value: float) -> None:
    if (
        ...
        raise EvaluationValidationError(
            "Minimum route accuracy must be within (0, 1]."
        )
...
DEFAULT_ROUTE_EVAL_CASES = (
    RouteEvalCase("uz_fast_greeting", "Salom", SuperAIRoute.FAST),
    RouteEvalCase("uz_fast_short", "Qisqa javob ber", SuperAIRoute.FAST),
    RouteEvalCase("uz_fast_python", "Python nima?", SuperAIRoute.FAST),
    RouteEvalCase("en_fast_tuple", "What is a tuple?", SuperAIRoute.FAST),
    RouteEvalCase("en_fast_translate", "Translate hello", SuperAIRoute.FAST),
    RouteEvalCase("en_fast_thanks", "Thank you", SuperAIRoute.FAST),
    RouteEvalCase(
        "uz_context_sources",
