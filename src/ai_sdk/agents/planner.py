...
class LLMAgentPlanner(BaseAgentPlanner):
    ...
    def __init__(
        ...
        *,
        ...
        if not isinstance(client, BaseLLMClient):
            raise TypeError(
                "Planner client must be a BaseLLMClient."
            )
        ...
        if (
            ...
            raise ValueError(
                "Maximum plan steps must be greater than zero."
            )
    ...
    def _parse_steps(self, response: str) -> list[str]:
        ...
        if (
            ...
            or set(payload) != {"steps"}
        ):
            raise PlanValidationError(
                "Planner response must contain only steps."
            )
        ...
        if len(raw_steps) > self.max_steps:
            raise PlanValidationError(
                "Planner returned too many steps."
            )
        ...
        if any(
            ...
            raise PlanValidationError(
                "Planner steps must be non-empty strings."
            )
