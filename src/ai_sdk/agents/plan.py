...
class PlanStep:
    ...
    def __post_init__(self) -> None:
        ...
        if (
            ...
            raise PlanValidationError(
                "Plan step description cannot be empty."
            )

        if not isinstance(self.status, PlanStatus):
            raise PlanValidationError(
                "Plan step status is invalid."
            )
        ...
        if self.outcome is not None and not isinstance(
            ...
            raise PlanValidationError(
                "Plan step outcome must be a string."
            )
...
class AgentPlan:
    ...
    def __init__(
        ...
        if not isinstance(goal, str) or not goal.strip():
            raise PlanValidationError(
                "Agent plan goal cannot be empty."
            )
        ...
        if len(step_ids) != len(set(step_ids)):
            raise PlanValidationError(
                "Agent plan step IDs must be unique."
            )

        if sum(
            step.status is PlanStatus.IN_PROGRESS
            for step in normalized_steps
        ) > 1:
            raise PlanValidationError(
    ...
    def status(self) -> PlanStatus:
        ...
        if (
            PlanStatus.IN_PROGRESS in statuses
            or PlanStatus.COMPLETED in statuses
        ):
    ...
    def current_step(self) -> PlanStep | None:
        return next(
            (
                step
                for step in self._steps
                if step.status is PlanStatus.IN_PROGRESS
            ),
            None,
        )
    ...
    def start_next(self) -> PlanStep:
        if self.current_step is not None:
            raise PlanValidationError(
                "A plan step is already in progress."
            )

        if self.status is PlanStatus.FAILED:
            raise PlanValidationError(
                "A failed plan cannot start another step."
            )
        ...
        raise PlanValidationError(
            "Agent plan has no pending steps."
        )
    ...
    def fail_current(self, error: str) -> PlanStep:
        if not isinstance(error, str) or not error.strip():
            raise PlanValidationError(
                "Plan step error cannot be empty."
            )
