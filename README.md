...
flowchart LR
    ...
    MODE -->|Adaptive Maximum| GEMINI[Gemini: evidence extraction]
    GEMINI --> CLAUDE[Claude: contradiction-aware analysis]
    CLAUDE --> OPENAI[OpenAI: final synthesis]
    MODE -->|Adaptive Auto| ROUTER[Capability router]
    ROUTER -->|Fast| OPENAI
    ROUTER -->|Context| CONTEXT[Gemini → OpenAI]
    ROUTER -->|Reasoning| REASON[Claude → OpenAI]
    ROUTER -->|Full| GEMINI
